...
3. **执行迁移**（按顺序；推荐 `sudo ./scripts/deploy.sh migrate` 一次跑齐 `migrations/[0-9][0-9][0-9]_*.sql`）：
   - `migrations/001_init.sql`
   - `migrations/002_user_identities.sql`
   - `migrations/003_user_profile.sql`（用户资料列；**可重复执行**，按列检测跳过）
   - `migrations/004_redemption_codes.sql`、`migrations/005_membership_recharge_records.sql`（兑换码、会籍充值审计）
   - `migrations/006_admin_login_ip_guards.sql`、`migrations/007_user_device_sessions.sql`（管理后台 IP 封禁、用户设备会话；`007` 为 `CREATE TABLE IF NOT EXISTS`）
   - `migrations/008_user_device_sessions_app_version.sql`（设备会话 `app_version`；**可重复执行**）
